...
const ProjectNavBar: React.FC<ProjectNavBarProps> = ({ 
  ...
  return (
    <div className="sticky bottom-0 z-20 bg-[var(--color-surface)]/90 backdrop-blur-md border-t border-[var(--color-border)]/50">
      <div className="flex items-center justify-between px-3 py-2 max-w-7xl mx-auto">
        <Link
          href={`/project/${slugify(prevProject.title)}`}
          className="flex items-center gap-2 text-sm text-[var(--color-muted-foreground)] hover:text-[var(--color-accent)] transition-colors group"
        >
        ...
        <span className="text-xs text-[var(--color-muted-foreground)]/50">
